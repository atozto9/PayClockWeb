...
describe('payCalculator', () => {
  ...
  it('starts premium overtime after distributed threshold', () => {
    ...
    expect(summary.dailyRequiredHours).toBeCloseTo(8, 4)
    expect(summary.dailyPremiumStartHours).toBeCloseTo(8.7, 4)
  ...
  it('keeps daily hours at zero when effective workdays are zero', () => {
    ...
    expect(summary.dailyRequiredHours).toBe(0)
    expect(summary.dailyPremiumStartHours).toBe(0)
  })

  it('does not pay hours between required and premium threshold', () => {
    const summary = summarizeMonth(
      '2026-08-01',
      [workRecord('2026-08-04', { endMinute: 18 * 60 + 40 })],
      makeSettings(),
      combineDayAndMinutes('2026-08-01', 0),
    )
  ...
  it('matches automatic break thresholds', () => {
    ...
    expect(
      breakdownForDay('2026-08-06', workRecord('2026-08-06', { endMinute: 13 * 60 }), settings, 24, combineDayAndMinutes('2026-08-06', 0))
        .autoBreakMinutes,
    ).toBe(0)
    expect(
      breakdownForDay('2026-08-06', workRecord('2026-08-06', { endMinute: 13 * 60 + 1 }), settings, 24, combineDayAndMinutes('2026-08-06', 0))
        .autoBreakMinutes,
    ).toBe(30)
    expect(
      breakdownForDay('2026-08-06', workRecord('2026-08-06', { endMinute: 17 * 60 + 30 }), settings, 24, combineDayAndMinutes('2026-08-06', 0))
        .autoBreakMinutes,
    ).toBe(30)
    expect(
      breakdownForDay('2026-08-06', workRecord('2026-08-06', { endMinute: 17 * 60 + 31 }), settings, 24, combineDayAndMinutes('2026-08-06', 0))
        .autoBreakMinutes,
    ).toBe(60)
  })
  ...
  it('estimates premium start during a running shift', () => {
    const records = [
      workRecord('2026-04-06', { startMinute: 7 * 60 + 33, endMinute: null, isRunning: true }),
    ...
    const summary = summarizeMonth(
      ...
      combineDayAndMinutes('2026-04-06', 12 * 60 + 31),
    )

    const breakdown = summary.days.find((day) => day.dayKey === '2026-04-06')
